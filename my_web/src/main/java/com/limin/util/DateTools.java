...
public class DateTools {
	...
	/**
	 * 获取当前日期并，格式化日期为字符串 "yyyyMMddhhmmss"
	 * @author dabin
	 * @return
	 */
	...
	public static String getPastTime(Date pastDate){
		...
		try{
			...
			currentCal.setTime(FORMAT.parse(FORMAT.format(currentCal.getTime())));
			...
			if (pastCal.before(currentCal)){
				...
				if (pastCal.before(currentCal)){
					...
					if (pastCal.before(currentCal)){
						...
						long timeLong = currentCal.getTime().getTime() - pastDate.getTime();
						...
						if (timeLong < 0){
							...
							if (pastCal.before(currentCal)){
								time = String.valueOf(timeLong / MINUTE) + "分钟前";
							} else{
								time = "刚刚";
							}
						}
	...
	public static String nDaysBetweenTwoDate(String firstString){
		...
		double number_s = (firstDate.getTime() - secondDate.getTime()) % (1000 * 60 * 60 * 24);
		if (number_s > 0){
			nDay = nDay + 1;
		}
		if (nDay > 1){
